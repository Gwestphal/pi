...
if(!empty($_GET['idmusica'])){
    $musica->carregarPorId($_GET['idmusica']);
}
...
<div class="container">
    <form class="form-horizontal" action="processamento.php?acao=salvar" method="post">
        <input type="hidden" name="idmusica" value="<?php echo $musica->getIdmusica(); ?>">

        <div class="form-group">
            <label for="artista" class="col-sm-2 control-label">Artista</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" id="artista" name="artista" value="<?php echo $musica->getArtista(); ?>">
            </div>
        </div>
        <div class="form-group">
